...
import { NguCarousel, NguCarouselService } from "@ngu/carousel";
...
@IonicPage({
  ...
  segment: "code_de_la_route"
})
@Component({
  selector: "page-codedelaroute",
  templateUrl: "codedelaroute.html"
})
export class CodedelaroutePage {
  @ViewChild("slides")
  slides: Slides;
  ...
  drop(e: any, reponseId: string) {
    ...
    $("#reponse-" + reponseId).addClass("selected");
  ...
  cancel() {
    if (this.questions[this.questionIndex].userReponse) {
      this.onSelect = false;
      $(
        "#reponse-" + this.questions[this.questionIndex].userReponse.id
